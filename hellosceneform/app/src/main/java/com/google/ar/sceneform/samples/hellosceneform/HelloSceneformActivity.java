...
public class HelloSceneformActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    configuration.setFocusMode(FocusMode.AUTO);
  ...
  void Update()
  {
      ...
      int cameraHeight = session.getCameraConfig().getImageSize().getHeight();
      int cameraWidth = session.getCameraConfig().getImageSize().getWidth();

      // RAYCAST
      try {
          Frame frame = arFragment.getArSceneView().getSession().update();
          HitResult hitResult = frame.hitTest(cameraWidth/2, cameraHeight/2).get(0);
